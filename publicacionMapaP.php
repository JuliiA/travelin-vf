...
<head>
	<?php
		...
		$sqlMapa0 = 'SELECT idEstableci, lat, lng FROM establecimiento WHERE idUsuario='.$idUsuario.' AND idEstado = 2';
									
		if(!($consultaMapa0 = mysqli_query($db->conectarse(), $sqlMapa0)))
		{
	...
	//echo('nom= '.$nombre.' , direc= '.$direccion.', desc='.$descripcion.', idus='.$idUsuario.', tipoest='.$tipoEstablecimiento.', ciud='.$ciudad.', prov='.$provincia);
			...
			{
				$RsMapa0 = mysqli_fetch_assoc($consultaMapa0);
				$filas0 = mysqli_num_rows($consultaMapa0);
				...
				if($filas0 == 0)
				{
					echo ('<html><head>
								<script type="text/javascript">
									...
									{
										alert("Antes de ubicar en el mapa debe crear el anuncio.");
									}
								</script>
								</head><body>
				...
				}else
					{
						if($filas0 == 1)
						{
							$idEstableci = $RsMapa0['idEstableci'];
							$lat = $RsMapa0['lat'];
							$lng = $RsMapa0['lng'];
						}
					}
									...
									<div>
										<div id="contFormPublicacion" class="contFormulario1">
						   					<div id="agrupFormPublicacion" name="agrupFormPublicacion" class="contAgrupForm">
									            ...
									            <form class="form-horizontal" action="publicacionMapaC.php" method="post" enctype="multipart/form-data">
													<fieldset>
										                ...
										                <div class="form-group sep2">
															<div class="labelTitFormP">
																<label class="control-label labelTitForm"><h4>Ubicación en el mapa</h4></label>
															</div>
														</div>
														...
														<div id="map_canvas" name="map_canvas" style="width:100%; height:400px;">
														</div>

														<br/>
														
														<input id="idEstableci" name="idEstableci" type="hidden" value="<?php echo $idEstableci; ?>">
														<input id="loglat" name="loglat" placeholder="latitud" class="" type="hidden" 
														<?php
															if($existe == 1)
															{
																echo 'value="'.$lat.'"';
														...
														?>>
														<input id="loglong" name="loglong" placeholder="longitud" class="" type="hidden"
														<?php
															if($existe == 1)
															{
																echo 'value="'.$lng.'"';
										                ...
										                <div class="pull-centro">
										                	<!--<input type="submit" id="btnEliminarMapa" name="btnEliminarMapa" value="Borrar Posición">
															<input type="submit" id="btnGuardar" name="btnGuardar" value="Guardar Posición" >
															<input type="button" id="btnCancelar" name="btnCancelar" value="Cancelar" onclick="">-->
															<input type="submit" id="btnFormMapa" name="btnFormMapa" value="Borrar Posición" >
															<input type="submit" id="btnFormMapa" name="btnFormMapa" value="Guardar Posición" >
															<input type="submit" id="btnFormMapa" name="btnFormMapa" value="Cancelar" >
										                </div>
