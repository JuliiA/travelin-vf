...
<head>
	<?php
		...
		$sqlParada0 = 'SELECT * FROM parada WHERE idDiario='.$idDiario;
		
		if(!($consultaParada0 = mysqli_query($db->conectarse(), $sqlParada0)))
		{
	...
	//echo('nom= '.$nombre.' , direc= '.$direccion.', desc='.$descripcion.', idus='.$idUsuario.', tipoest='.$tipoEstablecimiento.', ciud='.$ciudad.', prov='.$provincia);
		}else
			{
				...
				$filasP0 = mysqli_num_rows($consultaParada0);
														...
														<!--<section>-->
															<table class="table table-striped span3">
																...
																<?php
																	if($filasP0 == 0)
																	{
																		echo ('<tr><td>No se encontraron paradas guardadas.</td><td></td><td></td></tr>');
																	}else
																		{
																			while ($RsParada0 = mysqli_fetch_assoc($consultaParada0)) {
																				$fechaActual = $RsParada0['fecha'];
																				...
																				$estado = 'Pendiente';
																				if($RsParada0['idEstado'] == 5) {
																					$estado = 'Realizada';
																				}else 
																					{
																						if($RsParada0['idEstado'] == 2) {
																							$estado = 'En Curso';
																						}else
																							{
																								if($RsParada0['idEstado'] == 1) {
																									$estado = 'Pendiente';
																								}
																							}
																					}
																				...
																				echo '<tr>
																							<td><a href="verParada.php?num='.$RsParada0["idParada"].'">'.$RsParada0['descripcion'].'</a></td>
																							<td><a href="verParada.php?num='.$RsParada0["idParada"].'">'.$fechaFormateada.'</a></td>
																							<td><a href="verParada.php?num='.$RsParada0["idParada"].'">'.$estado.'</a></td>
																							<td><a href="mostrarParada.php?num='.$RsParada0["idParada"].'" target="_blank"><img src="imagenes/botonOjo.png" class="" alt="Ver la parada"/></a></td>
																					</tr>';
																			}
																		}
