...
<head>
	<?php
		...
		$sqlFotos0 = 'SELECT idEstableci, nombre FROM establecimiento WHERE idUsuario='.$idUsuario.' AND idEstado = 2';
		if(!($consultaFotos0 = mysqli_query($db->conectarse(), $sqlFotos0)))
		{
	...
	//echo('nom= '.$nombre.' , direc= '.$direccion.', desc='.$descripcion.', idus='.$idUsuario.', tipoest='.$tipoEstablecimiento.', ciud='.$ciudad.', prov='.$provincia);
			...
			{
				$RsFotos0 = mysqli_fetch_assoc($consultaFotos0);
				$filas0 = mysqli_num_rows($consultaFotos0);
				...
				if($filas0 == 0)
				{
					...
					{
						$idEstableci = $RsFotos0['idEstableci'];
						$nombreE = $RsFotos0['nombre'];
					}
			}
	?>
									...
									<div>
										<div id="contFormPublicacion" class="contFormulario1">
						   					<div id="agrupFormPublicacion" name="agrupFormPublicacion" class="contAgrupForm_">
									            ...
									            <form class="form-horizontal" action="publicacionGaleriaC.php" method="post" enctype="multipart/form-data">
													<fieldset>
										                ...
										                <div class="form-group sep2">
															<div class="labelTitFormP">
																<label class="control-label labelTitForm"><h4>Fotos de <?php echo $nombreE; ?></h4></label>
															</div>
														</div>
...
-->
														...
														<?php
															$sqlFotos1 = 'SELECT * FROM fotoEstableci WHERE idEstableci ='.$idEstableci;
														...
														//echo('nom= '.$nombre.' , direc= '.$direccion.', desc='.$descripcion.', idus='.$idUsuario.', tipoest='.$tipoEstablecimiento.', ciud='.$ciudad.', prov='.$provincia);
															}else
																{
																	...
																	if($filas1 == 0)
																	{
																		echo '<h3>El anuncio no posee fotos.</h3>';
																	}else
																		{
																			while ($RsFotos1 = mysqli_fetch_assoc($consultaFotos1))
																			{
																				echo '<div class="contGaleria2">
																						<a class="group2" href="galeriaEstablecimiento/'.$RsFotos1['nombre'].'" title="">
																							<img src="galeriaEstablecimiento/miniaturas/'.$RsFotos1['nombre'].'" class="imgGaleria1">
																						</a>
																					</div>';
																			}
																		}
