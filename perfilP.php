...
<head>
	<?php
		...
		}else
			{
				echo ('<html><head>
							<script type="text/javascript">
								...
								{
									alert("Debe loguearse para ver su perfil.");
								}
							</script>
						</head><body>
		...
		$sqlPerfil0 = 'SELECT * FROM usuario WHERE idUsuario='.$idUsuario;
		
		if(!($consultaPerfil0 = mysqli_query($db->conectarse(), $sqlPerfil0)))
		{
	...
	//echo('nom= '.$nombre.' , direc= '.$direccion.', desc='.$descripcion.', idus='.$idUsuario.', tipoest='.$tipoEstablecimiento.', ciud='.$ciudad.', prov='.$provincia);
		}else
			{
				...
				//$filasP0 = mysqli_num_rows($consultaPerfil0);
				$RsPerfil0 = mysqli_fetch_assoc($consultaPerfil0);

				$ciudad = $RsPerfil0['idCiudad'];
			}

/***********************************************************************************************/
		if(($ciudad != 0) && ($ciudad != 'NULL') && ($ciudad != ''))
		{
			$sqlPerfil1 = 'SELECT PA.id idPais, PR.id idProvincia, C.id idCiudad FROM pais PA INNER JOIN provincia PR ON PA.id = PR.idPais INNER JOIN ciudad C ON PR.id = C.idProvincia 
							WHERE C.id ='.$ciudad;
			
			if(!($consultaPerfil1 = mysqli_query($db->conectarse(), $sqlPerfil1)))
			{
				echo('Ocurrio un error ejecutando el query de busqueda de ubicación [' . mysqli_error() . ']');

			}else
				{
					$RsPerfil1 = mysqli_fetch_assoc($consultaPerfil1);

					$ciudadP = $ciudad;
					$provinciaP = $RsPerfil1['idProvincia'];
					$paisP = $RsPerfil1['idPais'];
				}
		}else
			{
				$ciudadP = 0;
...
*/
/*
			$("#ddlPaisP").ready(function () {
				$("#ddlPaisP option:selected").each(function () {
					elegido=$(this).val();
					elegCiudad = $('#ciuEleg').val();
					$.post("ddlPais.php", { elegido: elegido, elegCiudad: elegCiudad }, function(data){
						$("#ddlProvinciaP").html(data);
						$("#ddlCiudadP").html("");
					});
				});
			});
*/
			$("#ddlPaisP").change(function () {
				$("#ddlPaisP option:selected").each(function () {
					elegido=$(this).val();
					elegCiudad = $('#ciuEleg').val();
					$.post("ddlPais.php", { elegido: elegido, elegCiudad: elegCiudad }, function(data){
...
*/
	...
	<script type="text/javascript">
		function sugerirUs(str) {
			...
			xmlhttp.open("GET", "validarNick.php?n="+str, true);
			xmlhttp.send();
									...
									<div>
										<div id="contFormPublicacion" class="contFormulario1">
						   					<div id="agrupFormPublicacion" name="agrupFormPublicacion" class="contAgrupForm_">
									            ...
									            <form class="form-horizontal" action="perfilC.php" method="post" enctype="multipart/form-data">
													<fieldset>

										                <!-- Form Name -->
										                <legend id="tituloForm">Perfil.</legend>
										                ...
										                <div class="form-group sep2">
										                	<div class="labelIzqFormP">
																<label class="control-label labelFormP" for="nombreU">Nombre</label>  
															</div>
															<div class="elementFormP">
																<input type="text" id="nombreU" name="nombreU" value="<?php echo($RsPerfil0['nombre']); ?>" >
										                ...
										                <div class="form-group sep2">
										                	<div class="labelIzqFormP">
																<label class="control-label labelFormP" for="apellidoU">Apellido</label>  
															</div>
															<div class="elementFormP">
																<input type="text" id="apellidoU" name="apellidoU" value="<?php
																													if($RsPerfil0['apellido'] != 'NULL') {
																														echo $RsPerfil0['apellido'];
																													} ?>" >
										                ...
										                <div class="form-group sep2">
										                	<div class="labelIzqFormP">
																<label class="control-label labelFormP" for="nickU">Nombre de Usuario</label>  
															</div>
															<div class="elementFormP">
																<input class="txtSugerido_" type="text" id="nickU" name="nickU" value="<?php echo $RsPerfil0['nick']; ?>" onkeyup="sugerirUs(this.value)" >
																
										                    	<div class="contImgNick" id="txtHint"></div>
															</div><!--<div class="divImgNick"><img class="imgNick" src="imagenes/Tilde_Verde.png" /></div>-->
										                </div>
														...
														<div class="form-group sep2">
															<div class="labelIzqFormP">
																<label class="control-label labelFormP" for="ddlPaisP">Pais</label>
															</div>
															<div class="elementFormP">
																<select id="ddlPaisP" name="ddlPaisP" class="form-control_">
																	<?php
																		echo '<option value="0" ';
																		if($paisP == 0 || $paisP == '')/*	AGREGAR EL SI ES is_null(var)	*/
																		{
																			echo 'selected';
																		}
																		echo '>Elija un pais.</option>';

																		$sqlPA = 'SELECT id, descripcion FROM pais';
																		$consultaPA = mysqli_query($db->conectarse(), $sqlPA) or die(mysqli_error());
																		while ($RsPA = mysqli_fetch_assoc($consultaPA)) {
																	?>
																			<option title="<?php echo $RsPA["descripcion"]; ?>" value="<?php echo $RsPA["id"]; ?>"
																																		<?php 
																																			if($RsPA["id"] == $paisP)
																																			{
																																				echo ("selected");
														...
														<div class="form-group sep2">
															...
															<div class="elementFormP">
																<select id="ddlProvinciaP" name="ddlProvinciaP" class="form-control_">
																	<?php
																		echo '<option value="0" ';
																		if($provinciaP == 0 || $provinciaP == '')/*	AGREGAR EL SI ES is_null(var)	*/
																		{
																			echo 'selected';
																		}
																		...
																		$sqlPR = 'SELECT id, descripcion FROM provincia WHERE idPais ='.$paisP;
										                ...
										                <div class="form-group sep2">
															...
															<div class="elementFormP">
																...
																<?php
																	//if ($existeP == 1)
																	//{
																		$sqlCI = 'SELECT id, descripcion FROM ciudad WHERE idProvincia = '.$provinciaP;
																		$resulCI = mysqli_query($db->conectarse(), $sqlCI) or die(mysqli_error());

																		echo '<option value="0" ';
																		if($ciudadP == 0 || $ciudadP == '')/*	AGREGAR EL SI ES is_null(var)	*/
																		{
																			echo 'selected';
																		}
																		...
																		while($RsCI = mysqli_fetch_array($resulCI))
																		{
																			...
																			//echo '>'.$RsCI["id"].' - '.$RsCI["descripcion"].'</option>';
														...
														<div class="form-group sep2">
															<div class="labelIzqFormP">
																<label class="control-label labelFormP" for="fileFotoPerfilActual">Foto Actual del Perfil</label>
															</div>
															<div class="elementFormP">
																<div class="contFotoP">
																	<div class="fotoP">
																		<img src="<?php
																						$foto = $RsPerfil0['nombreFotoPerfil'];
																						if(($foto == '') || ($foto == 'NULL'))
																						{
																							echo ('imagenes/imgPerfil/no_avatar.gif');
																						}else
																							{
																								echo 'imagenes/imgPerfil/'.$foto;
																							}
														...
														<div class="form-group sep2">
															<div class="labelIzqFormP">
																<label class="control-label labelFormP" for="fileFotoPerfil">Cambiar Foto del Perfil</label>
															</div>
															<div class="elementFormP">
																<input id="fileFotoPerfil" name="fileFotoPerfil" type="file">
										                ...
										                <div class="pull-centro">
										                	<input type="submit" id="btnPerfil" name="btnPerfil" value="Guardar" >
										                	<?php
