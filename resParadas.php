...
<head>
	<?php
		...
		$txtParada = $_POST['txtBusquedaParadas'];

		$sqlParada1 = 'SELECT * FROM parada WHERE nombre LIKE "%'.$txtParada.'%" OR descripcion LIKE "%'.$txtParada.'%"';
		
		if(!($consultaParada1 = mysqli_query($db->conectarse(), $sqlParada1)))
		{
	...
	//echo('nom= '.$nombre.' , direc= '.$direccion.', desc='.$descripcion.', idus='.$idUsuario.', tipoest='.$tipoEstablecimiento.', ciud='.$ciudad.', prov='.$provincia);
		}else
			{
				...
				$filasP1 = mysqli_num_rows($consultaParada1);
									...
									<div>
										<div id="contFormPublicacion" class="contFormulario1">
						   					<div id="agrupFormPublicacion" name="agrupFormPublicacion" class="contAgrupForm_">
												...
												<!--<form class="form-horizontal" action="paradaNuevaP.php" method="post" enctype="multipart/form-data">-->
													<fieldset>
										                ...
										                <div class="contEncabezadoTabla1">
															<div class="tituloTabla2">
																<h4>Buscar Parada.</h4>
																<div class="contBuscador1">
																	<form class="" action="resParadas.php" method="post" enctype="multipart/form-data">
																		<input class="inputBusqueda1" type="text" id="txtBusquedaParadas" name="txtBusquedaParadas" />
																		<br>
																		<div class="contBtn1">
																			<input type="submit" id="btnBuscarParada" name="btnBuscarParada" value="Buscar Parada" >
																			<br>
																		</div>
																	</form>
																</div>
															...
															<div class="btnTituloTabla1">
																<input type="button" id="btnParadaNueva" name="btnParadaNueva" value="Crear Parada" onclick="location.href='paradaNuevaP.php?num=<?php echo($idDiario); ?>'">
															</div>
															-->
														</div>
														...
														<legend id="tituloForm"></legend>

														<div class="contEncabezadoTabla1">
															<div class="tituloTabla1">
																<h4>Paradas Resultantes.</h4>
															</div>
															<div class="btnTituloTabla1">
																<input type="hidden" id="btnParadaNueva" name="btnParadaNueva" value="Crear Parada" onClick="">
															</div>
														</div>
														...
														<!--<section>-->
															<table class="table table-striped span3">
																...
																<?php
																	if($filasP1 == 0)
																	{
																		echo ('<tr><td>No se encontraron paradas con el texto introducido.</td><td></td><td></td></tr>');
																	}else
																		{
																			while ($RsParada1 = mysqli_fetch_assoc($consultaParada1)) {
																				$fechaActual = $RsParada1['fecha'];
																				$fechaM = strtotime($fechaActual);
																				$aaaa = date("Y", $fechaM);
																				$mm = date("m", $fechaM);
																				$dd = date("d", $fechaM);
																				
																				$fechaFormateada = $dd.'/'.$mm.'/'.$aaaa;
																				
																				$estado = 'Pendiente';
																				if($RsParada1['idEstado'] == 5) {
																					$estado = 'Realizada';
																				}else 
																					{
																						if($RsParada1['idEstado'] == 2) {
																							$estado = 'En Curso';
																						}else
																							{
																								if($RsParada1['idEstado'] == 1) {
																									$estado = 'Pendiente';
																								}
																							}
																					}
																				...
																				echo '<tr>
																							<td><a href="mostrarParada.php?num='.$RsParada1["idParada"].'">'.$RsParada1['nombre'].'</a></td>
																							<td><a href="mostrarParada.php?num='.$RsParada1["idParada"].'">'.$fechaFormateada.'</a></td>
																							<td><a href="mostrarParada.php?num='.$RsParada1["idParada"].'">'.$estado.'</a></td>
																							<td><a href="mostrarParada.php?num='.$RsParada1["idParada"].'" target="_blank"><img src="imagenes/botonOjo.png" class="" alt="Ver la parada"/></a></td>
																					</tr>';
																			}
																		}
